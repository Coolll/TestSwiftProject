...
public extension String {
    ...
    var isAlphaNumeric: Bool {
        ...
        return comps.joined().count == 0 && hasLetters && hasNumbers
    }
    ...
    var dateTime: Date? {
        ...
        return formatter.date(from: trimStr)
    }
    #endif
...
public extension String {
    func width(font: UIFont?) -> CGFloat {
        self.boundingRect(with: CGSize(width: CGFloat(MAXFLOAT), height: CGFloat(MAXFLOAT)), options: .usesLineFragmentOrigin, attributes: [NSAttributedString.Key.font: font as Any], context: nil).width
    }
    ...
    /// parse ISO formatter:  2022-06-16T16:00:00.0000Z(with fractional seconds)   or    2022-06-16T16:00:00Z (without fractional seconds)
    func dateWithISOFormatter() -> Date? {
...
public extension String {
    ...
    func transformToPinWithouBlank() -> String {
        var pinyin = self.transformToPinyin()
        pinyin = pinyin.replacingOccurrences(of: " ", with: "")
        return pinyin
    }

    func isMatch(searchWithPinyin: String) -> Bool {
        if self.contains(searchWithPinyin, caseSensitive: false) {
            return true
        }

        if self.isIncludeChinese() && !searchWithPinyin.isIncludeChinese() {
            if self.transformToPinyin().contains(searchWithPinyin, caseSensitive: false) {
                return true
            }
            if self.transformToPinWithouBlank().contains(searchWithPinyin, caseSensitive: false) {
                return true
            }
        }
        return false
    ...
    func decimalToEmojiUnicode() -> String {
        // 由于后端有几率传回多个icon的参数，样式："[:129489]‍[:127979]"
        let pattern = "\\[:[0-9]+\\]"
        ...
        var lastLocation = 0
        for one in matches ?? [] {
            if one.range.location > lastLocation {
                let range = NSRange.init(location: lastLocation, length: one.range.location - lastLocation)
                let emoji = self.substring(with: range)
            
                uniCodeArr += emoji.unicodeScalars
                lastLocation = one.range.location
            }
            
            let subStr = String(self.substring(with: one.range).components(separatedBy: CharacterSet.decimalDigits.inverted)
                .joined())
            if let emoji = decimalToEmoji(decString: subStr) {
                uniCodeArr.append(emoji)
            }
            
            
            lastLocation = one.range.location + one.range.length
        }
        
        if self.length > lastLocation {
            let range = NSRange.init(location: lastLocation, length: self.length - lastLocation)
            let emoji = self.substring(with: range)
        
            uniCodeArr += emoji.unicodeScalars
        }
        
        if uniCodeArr.count > 0 {
            return String(String.UnicodeScalarView(uniCodeArr))
        }
        return self
    }
    
    // decimal Format is [:127819]
    func decimalToEmoji(decString: String) -> UnicodeScalar? {
        // 转为16进制
        if let charAsInt = Int(decString, radix: 10) {
            return UnicodeScalar(charAsInt)
        }
        return nil
    }
